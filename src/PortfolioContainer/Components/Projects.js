import React from 'react';
import './project.css'

const Projects = () => {
    return (
        <>
            <div className="resume-details-container">
                <div className="resume-detail-main resume-detail-main4">
                    <div className="resume-college-info">
                        <h5>Aarvis </h5>
                        <p className="tecUsed">
                            <span>Technology Used :- </span>
                            Hbs, HTML, Css, JavaScript, Express Js, Node js 
                        </p>
                        <p className="tecUsed">
                            <span>Description :- </span>
                            The name Aarvis inspired by Jarvis(Iron Man). Like Jarvis, It listen and give response according to your question.
                        </p>
                    </div>
                    <div className="resume-clg-time resume-clg-time4">
                        <div><a href="https://aarvis.herokuapp.com/" target="_blank" rel="noopener noreferrer">View</a></div>
                        
                    </div>
                </div>
                <div className="resume-detail-main resume-detail-main4">
                    <div className="resume-college-info">
                        <h5>Live Tempreture Website </h5>
                        <p className="tecUsed">
                            <span>Technology Used :- </span>
                           HTML, Css, JavaScript, Express Js, Node js, Tempreture API.
                        </p>
                        <p className="tecUsed">
                            <span>Description :- </span>
                            It is basically used to watch live tempreture of any place in the world by just simply giving the name of city.
                        </p>
                    </div>
                    <div className="resume-clg-time resume-clg-time4">
                        <div><a target="_blank" href="https://livetempreture.herokuapp.com/" rel="noopener noreferrer">View</a></div>
                    </div>
                </div>
                <div className="resume-detail-main resume-detail-main4">
                    <div className="resume-college-info">
                        <h5>First Mern Website </h5>
                        <p className="tecUsed">
                            <span>Technology Used :- </span>
                            React Js, Css, JavaScript, Express Js, Node js, Mongo Db, Mongoose
                        </p>
                        <p className="tecUsed">
                            <span>Description :- </span>
                            It was my First Mern Website. It is just a template website which i create for enhance my Mern Skils
                        </p>
                    </div>
                    <div className="resume-clg-time resume-clg-time4">
                        <div><a target="_blank" href="https://myfirstmernweb.herokuapp.com/" rel="noopener noreferrer">View</a></div>
                    </div>
                </div>
                <div className="resume-detail-main resume-detail-main4">
                    <div className="resume-college-info">
                        <h5>Snake Game</h5>
                        <p className="tecUsed">
                            <span>Technology Used :- </span>
                            HTML, Css, JavaScript, Github
                        </p>
                        <p className="tecUsed"> 
                            <span>Description :- </span>
                            It is a Snake Game.
                        </p>
                    </div>
                    <div className="resume-clg-time resume-clg-time4">
                        <div><a target="_blank" href="https://aman97703.github.io/snakegame" rel="noopener noreferrer">View</a></div>
                    </div>
                </div>
               
            </div>
        </>
    )
}

export default Projects
